...
from Tsunami_Inundation_Run_Updated_Again import Perform_Entire_Inundation
import gc

folder = "D:/Nationwide_Tsunami_Inundation/FABDEM_2_Latest_Inundation_Results"

# Path to the DEM raster file
dem_raster_path = 'D:/Nationwide_Tsunami_Inundation/DEM_Data/Combined_West_Coast_Southland_LiDAR_and_FABDEM.tif'
# Path to the polygon bounding boxes
shapefile_path = 'D:/Nationwide_Tsunami_Inundation/DEM_Data/DEM_Region_Extents/FABDEM/Southland_FABDEM_LiDAR_Merged.shp'

# Open the shapefile with the attribute table
gdf = gpd.read_file(shapefile_path)

Roughness_Polygons = gpd.read_file('D:/Nationwide_Tsunami_Inundation/lris-lcdb-v50-land-cover-database-version-50-mainland-new-zealand-SHP/updated-lcdb-v50-land-cover-database-version-50-mainland-new-zealand-with-roads-latest-roughness-additions.shp')
...
with rasterio.open(dem_raster_path) as src:
    ...
    for index, row in gdf.iterrows():
        try:
            ...
            os.makedirs(f"{folder}/{Region}", exist_ok=True)

            # Create a folder based on "Northland_" followed by the 'id' name within the output directory
            folder_path = f"{folder}/{Region}/{Area}"

            segments_completed = os.listdir(f"{folder}/{Region}")
            ...
            output_path = f"{folder}/{Region}/{Area}/{Area}_DEM.tif"
            ...
            del clipped_dem, clipped_transform, clipped_profile, DEM
            ...
            Perform_Entire_Inundation(folder_path, Region, Area, Original_Area, Htsunami_Heights, folder, Roughness_Polygons)
            
            # Clean up after processing each segment
            del Htsunami_Heights, geometry, Original_Area, Area, Region, region_parts, first_part, second_part, folder_path, segments_completed, output_path

            # Force garbage collection
            gc.collect()
        
        except Exception as e:
            print(f"Error processing segment {Area}: {e}")
            continue
