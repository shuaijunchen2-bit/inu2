...
def Perform_Entire_Inundation (folder_path, Region, Area, Original_Area, Htsunami_Heights, folder, Roughness_Polygons):
    ...
    import os
    from Functions import mask_of_polygon, inundation_extent, Find_Shoreline, TwoD_Crawl
    ...
    bounds = DEM.bounds
    #west = bounds[0], north = bounds[1], east = bounds[2], south = bounds[3]
    polygon = gpd.GeoDataFrame({"id":1,"geometry":[box(*bounds)]}, crs="EPSG:2193")
    #polygon.to_file('extent.shp')
    ...
    Elevation_Data = Elevation_Data - (Elevation_of_Survey_point_NZVD - Chart_Datum_Below_Survey_Point)
    ...
    '''
    print("Defining Inital Water Level")
    ...
    if Herbaceous_Saline_Vegetation_Present == 1:
        ...
        if buff_Herbaceous_Saline_Vegetation.empty == False:
    ...
    a[Initial_Water_Level > 0] = 2000
    Fill_to_the_MHWS = Initial_Water_Level.astype(int)  
    
    Tide_Raster = rio.open(f"D:/Nationwide_Tsunami_Inundation/Inundation_Results/{Region}/{Area}/Initial_Water_Level.tif" ,
                                  'w',
                                  driver='GTiff',
                                  height = Initial_Water_Level.shape[0],
                                  width = Initial_Water_Level.shape[1],
                                  count = 1,
                                  dtype = Initial_Water_Level.dtype,
                                  crs = 2193,
                                  transform = DEM.transform,
                                  )
    ...
    Roughness_Raster = rio.open(f"D:/Nationwide_Tsunami_Inundation/Inundation_Results/{Region}/{Area}/Roughness_Raster.tif" ,
                                  'w',
                                  driver='GTiff',
                                  height = a.shape[0],
                                  width = a.shape[1],
    ...
    Roughness_Raster.close() 
    del Roughness_Raster
    
    Water_Fill = local_buffed_river_mask + local_buffed_estuary_mask + Fill_to_the_MHWS
    Tide_Raster = rio.open(f"D:/Nationwide_Tsunami_Inundation/Inundation_Results/{Region}/{Area}/Water_Fill.tif" ,
                                  'w',
                                  driver='GTiff',
                                  height = Water_Fill.shape[0],
                                  width = Water_Fill.shape[1],
                                  count = 1,
                                  dtype = Water_Fill.dtype,
                                  crs = 2193,
                                  transform = DEM.transform,
                                  )
    ...
    Shoreline_Raster = rio.open(f"D:/Nationwide_Tsunami_Inundation/Inundation_Results/{Region}/{Area}/Shoreline_Raster.tif" ,
                                  'w',
                                  driver='GTiff',
                                  height = Shoreline_Boolean.shape[0],
                                  width = Shoreline_Boolean.shape[1],
                                  count = 1,
                                  dtype = 'int32',
                                  crs = 2193,
                                  transform = DEM.transform,
    ...
    #     Inundation_Raster = rio.open(f'D:/Nationwide_Tsunami_Inundation/Inundation_Results/{Region}/{Area}/{Area}_H{return_period}y{percentile}p_{Htsunami}m_10mResolution.tif',
    #                                  'w',
    #                                  driver='GTiff',
